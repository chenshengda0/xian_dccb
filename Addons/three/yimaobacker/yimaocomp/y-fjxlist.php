...
if($_POST['act']=='删除所选'){
	...
	$db->yiquery("select * from ymue where ue002='$_username' and ue011=0 and (ue008=0 or ue008=2) and ue001 in($u_id)");
	foreach ($db->rs as $k => $v) {
		if($v['ue008']==2){
			$db->yiexec('update ymue set ue008=3 where ue001='.$v['ue001']);
		}else{
			$db->yiexec('update ymue set ue008=1 where ue001='.$v['ue001']);
		}
		yimao_writelog('删除发件箱邮件:'.$v["ue005"]);
	}
...
$yimao_arr['yimao_option'].='<a href="'.YMCURL.'sjxlist" class="yimaoabtn">返回收件箱</a><a href="'.YMCURL.'sendemail" class="yimaoabtn">返回发送邮件</a><input type="submit" name="act" value="删除所选" class="yimaoabtn"   onclick="if(confirm(\'您确定要删除所选邮件吗?\')){artload(\'删除邮件中,请耐心等待！\')}else{return false}"><a href="'.getqueurl('option',1).'" class="'.geteqval(array($option,1,'yimaoaselbtn','yimaoabtn')).'">今日发送</a><a href="'.getqueurl('option',2).'" class="'.geteqval(array($option,2,'yimaoaselbtn','yimaoabtn')).'">所有邮件</a>';
...
if($quser){
	if(strstr($quser,'-')){
		...
		$condition.=" and (ue003 like '%".$quser."%' or ue005 like '%".$quser."%')";
	}
}
...
$yimao_arr['yimao_th']='<th><input type="checkbox" onclick="javascript:checkall(\'cbid[]\')"></th><th>序号</th><th>邮件标题</th><th>邮件类型</th><th>发送会员</th><th>未阅回复</th><th>发送时间</th><th>操作</th>';

$psql="select count(0) from ymue $condition";
$db->yiquery($psql,3);
$pcount=$db->fval;
$pagecount=$pagecount;   //每页条数 
$astr="&yimao=fjxlist&pagecount=$pagecount&orderby=$orderby&option=$option&quser=$quser";	  //Url
...
$yimao_arr['yimao_title']=getmenu($YCF['menu'],'fjxlist');
