...
if($_POST['submit']=='删除申请'){
	if(empty($_POST["cbid"])) yimao_automsg('请先选择要删除的记录！',$YCF['curl'],1);
	$u_id=implode(",",$_POST["cbid"]);

	$db->yiexec("update ymuv set uv038=0,uv039=null where uv038=1 and uv001 in($u_id)");
	yimao_writelog('删除服务中心申请');
	yimao_automsg('删除申请成功！',$YCF['curl']);
}

if($_POST['submit']=='确认申请'){
	if(empty($_POST["cbid"])) yimao_automsg('请先选择要确认的记录！',$YCF['curl'],1);
	$u_id=implode(",",$_POST["cbid"]);

	$db->yiexec("update ymuv set uv038=2 where uv038=1 and uv001 in($u_id)");
	yimao_writelog('确认服务中心申请');
	yimao_automsg('确认申请成功！',$YCF['curl']);
}
...
$yimao_arr['yimao_option'].='<input type="submit" name="submit"   onclick="return boxcheck(\'请先选择要确认的申请\',\'您确定要通过吗?\',\'确认正在执行中,请耐心等待！\');"  value="确认申请" class="yimaoabtn"><input type="submit" name="submit"   onclick="return boxcheck(\'请先选择要删除的记录\',\'您确定要删除申请吗?\',\'删除正在执行中,请耐心等待！\');"  value="删除申请" class="yimaoabtn"><a href="'.getqueurl('option',1).'" class="'.geteqval(array($option,1,'yimaoaselbtn','yimaoabtn')).'">今日新增</a><a href="'.getqueurl('option',2).'" class="'.geteqval(array($option,2,'yimaoaselbtn','yimaoabtn')).'">全部</a>';
...
$yimao_arr['yimao_th']='<th><input type="checkbox" onclick="javascript:checkall(\'cbid[]\')"></th><th>序号</th><th>账号</th><th>姓名</th><th>级别</th><th>推荐人</th><th>推荐数</th><th>申请时间</th>';

$psql="select count(0) from ymuv $condition";
$db->yiquery($psql,3);
$pcount=$db->fval;
$pagecount=$pagecount;   //每页条数 
$astr="&yimao=bdyeno&pagecount=$pagecount&orderby=$orderby&option=$option&quser=$quser";	  //Url
...
foreach ($db->rs as $k => $v) {
	...
	$yimao_arr['yimao_tr'].='<tr class="levelcolor'.$rm['um004'].'">
						...
						<td>'.$v['uv016'].'</td>
						<td>'.formatdate($v['uv039']).'</td>
						</tr>';
}


$yimao_arr['yimao_count']=$pcount;
$yimao_arr['yimao_title']=getmenu($YCF['menu'],'bdyeno');
