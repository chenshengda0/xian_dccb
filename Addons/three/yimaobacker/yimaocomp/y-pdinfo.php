...
$db->yiquery("select * from ymugp where gp011=0 and gp001=".$edid,2);
...
echo '<div class="main-r-body"><form method="post" action="'.geturl().'">
		<div class="main-r-item"><a href="'.YMCURL.'cunklist" class="yimaoabtn">返回</a></div>
...
?>
                        <tr>
                            <td width="25%" valign="top" align="center" colspan="4" style="background:#E5E5E5;font-size:16px;color:blue">您距离开始配对时间还有：
<div class="J_left_time dsj" rel="<?php echo strtotime("+".$YCF["jjcunpei"]." minute",$shijian)-time()?>" id="dsj" >&nbsp;
...
<script type="text/javascript">
 ...
 <?php 
     echo getcunstatus(array("pd009"=>$v["pd009"],"pd008"=>$v["pd008"],"pd011"=>$v["pd011"]));   
?>    
                            </td>
 ...
 <?php 
     ...
     if(empty($v["img"])&&empty($v["pd008"])&&$v["pd009"]==0){
 ?>                          
&nbsp;未上传图片,取款人未确认
  <?php  }elseif(!empty($v["img"])&&empty($v["pd008"])&&$v["pd009"]==0){?>
 &nbsp;等待存款人确认,<a href="javascript:artload1('<img src=<?php echo YMURL.'yimaoupfile/'.$v["img"]?>>');">查看上传</a>

  <?php  }elseif(!empty($v["img"])&&!empty($v["pd008"])&&$v["pd009"]==0){?>
 &nbsp;等待取款人确认,<a href="javascript:artload1('<img src=<?php echo YMURL.'yimaoupfile/'.$v["img"]?>>');">查看上传</a>
...
&nbsp;未上传图片,取款人已确认
 ...
 <?php  }elseif(!empty($v["img"])&&$v["pd009"]==1){?>
 &nbsp;取款人已确认,<a href="javascript:artload1('<img src=<?php echo YMURL.'yimaoupfile/'.$v["img"]?>>');">查看上传</a>
 <?php }?> 
