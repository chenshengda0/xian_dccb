...
function highlight_subnav(url) {
    $('.acc').find('a[href="' + url + '"]').parents('ul').attr('style','display:block;');
    $('.acc').find('a[href="' + url + '"]').attr('style','color:#F5B201;');
    $('.acc').find('a[href="' + url + '"]').parents('ul').prev('a').addClass('one-hover');
}
...
$(function(){
	...
	$(".ajax-get").click(function () {
		 var self = $(this);
		 $.get(self.attr("href"),success, "json");
		 return false;
	});
...
function highlightRows() {
	  var rows  = $('.t_c').find('tr');;
	  for(var i=0; i<rows.length; i++) {
		  if(i!=0){
		  //鼠标经过时高亮显示
		  $(rows[i]).hover(
			  function(){
			    $(this).addClass("onbgcolor");
			  },
			  function () {
			    $(this).removeClass("onbgcolor");
			  });
		  	//鼠标经过时高亮显示
			  $(rows[i]).click(
				 function(){
					 $(this).addClass("cbgcolor"); 
					 $(this).siblings().removeClass("cbgcolor"); 
				}); 
		  }
	  }
}
