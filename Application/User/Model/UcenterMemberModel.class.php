...
class UcenterMemberModel extends Model
{
    ...
    protected $_validate = array(
       ...
       array('password2', '6,30', -4, self::EXISTS_VALIDATE, 'length'), //密码长度不合法
    ...
    protected $_auto = array(
        ...
        array('password2', 'think_ucenter_md5', self::MODEL_BOTH, 'function', UC_AUTH_KEY),
    ...
    public function register( $data )
    {
    	...
    	unset($data['isadmin']);
    	$usercenter_data['username'] = $data['usernumber'];
    	$usercenter_data['password'] = $data['psd1'];
    	$usercenter_data['password2'] = $data['psd2'];
    	$usercenter_data['email'] = $data['email'];
    	$usercenter_data['mobile'] = $data['mobile'];
    	
        /* 添加用户 */
        $usercenter_member=$this->create($usercenter_data);
        
       // return $usercenter_data;
        if ($usercenter_member) {
            $result=D('Home/Member')->registerMember($data);
    ...
    public function login($username, $password, $type = 1)
    {
        ...
        $uinfo = M('member')->where($map)->getField('usernumber');
        
        $where['username'] = $uinfo;
    ...
    public function updateMember( $uid,$data )
    {
    	...
    	$result=D('Home/Member')->updateMember($uid,$data);
    	if($result){
    		if(isset($data['email'])){
    			$this->where(array('id'=>$uid))->setField('email',$data['email']);
    		}
    		if(isset($data['mobile'])){
    			$this->where(array('id'=>$uid))->setField('mobile',$data['mobile']);
    		}
    	}
    	return $result ;
    }
     ...
     * 更新用户密码
     */
    public function updateMemberpsd( $uid,$data )
    {

		
    	if(!empty($data['password'])){
    	    $udata['password'] = think_ucenter_md5($data['password'],UC_AUTH_KEY);
    	    $datac['psd1'] = $data['password'];
    	}
    	if(!empty($data['password2'])){
    	    $udata['password2'] = think_ucenter_md5($data['password2'],UC_AUTH_KEY);
    	    $datac['psd2'] = $data['password2'];
    	}
    	$usercenter_id['id']=$uid ;
    	$uid = M('Member')->where(array('uid'=>$uid))->save($datac);
      	
    	$id = $this->where($usercenter_id)->save($udata);
 ...
 /**
    ...
    {
        $map = array();
        $map['wechat_open_id']=$openid;

        /* 获取用户数据 */
        $user = M("Member")->where("wechat_open_id='$openid'")->find();
    ...
    protected function updateLogin($uid)
    {
        ...
        $this->save($data);
        $this->where(array('id'=>$uid))->setInc('login_count',1);
    }
    ...
    public function  jiami($data){
    	$data['psd1']=think_ucenter_md5($data['psd1'],UC_AUTH_KEY);
    	$data['psd2']=think_ucenter_md5($data['psd2'],UC_AUTH_KEY);
    	return $data;
    }
