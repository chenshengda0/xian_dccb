...
class AuthManagerController extends AdminController
{
    ...
    public function user($group_id)
    {
        ...
        $model = M()->table($l_table . ' m')->join($r_table . ' a ON m.mid=a.uid');
        $_REQUEST = array();
        $list = $this->lists($model, array('a.group_id' => $group_id, 'm.status' => array('egt', 0)), 'm.mid asc', null, 'm.mid,m.mname,m.last_login_time,m.last_login_ip,m.status');
    ...
    public function category()
    {
        ...
        $group_list = D('Category')->getTree(0);
    ...
    public function group()
    {
        ...
        $nickname = M('Manager')->where(array('mid'=>$uid))->getField('mname');
    ...
    public function addToGroup()
    {
        ...
        if (is_numeric($uid)) {
            ...
            if (!M('Manager')->where(array('mid' => $uid))->find()) {
                $this->error('管理员用户不存在');
            }
        }
