...
class DatabaseController extends AdminController{
    ...
    private function filterTable($list){
    	$prefix = C('DB_PREFIX');
    	$filter_arr = array("{$prefix}bonus_count","{$prefix}bonus_rule","{$prefix}bonus_total","{$prefix}finance",
    						"{$prefix}liuyan","{$prefix}member","{$prefix}money_change","{$prefix}recharge",
    						"{$prefix}ucenter_member","{$prefix}withdrawal");
    	$newlist = array();
    	foreach ($list as $val){
    ...
    public function uploadData(){
    	...
    	if($info){
    		...
    		$return['url'] = U('index',array('type'=>'import'));
    	} else {
    		$return['status'] = 0;
