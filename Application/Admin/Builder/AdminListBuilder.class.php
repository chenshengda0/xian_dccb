...
class AdminListBuilder extends AdminBuilder
{
     ...
     * @return $this
     */
    public function keyLinkStatus($name, $title, $getUrl)
    {
    	//如果getUrl是一个字符串，则表示getUrl是一个U函数解析的字符串
    	if (is_string($getUrl)) {
    		$getUrl = $this->createDefaultGetUrlFunction($getUrl);
    	...
    	//添加key
    	return $this->key($name, $title, 'linkstatus', $getUrl);
    }
    ...
    public function keyDoAction($getUrl, $text, $title = '操作',$type='')
    {
        //获取默认getUrl函数
        if (is_string($getUrl)) {
            $getUrl = $this->createDefaultGetUrlFunction($getUrl);
    ...
    public function keyDoActionEdit($getUrl, $text = '编辑',$type='tpl')
    {
        return $this->keyDoAction($getUrl, $text,'操作',$type);
    }
    
    
    public function keyDoActionInfo($getUrl, $text = '详情')
    {
    ...
    public function display($current='')
    {
        ...
        $this->convertKey('link', 'html', function ($value, $key, $item) {
        	...
        	//link转换为html
        	$this->convertKey('linkstatus', 'html', function ($value, $key, $item) {
        		$value = htmlspecialchars($value);
        		$getUrl = $key['opt'];
        		$url = $getUrl($item);
        		return "<a class='ajax-get' href=\"$url\">$value</a>";
        	});
        ...
        $this->convertKey('doaction', 'html', function ($value, $key, $item) {
            ...
            foreach ($actions as $action) {
                ...
                if($action['type']=='tpl'){
                	...
                	$result[] = "<a onclick='conf(this,\"{$linkText}\",\"{$url}\");' href='javascript:void(0);' >$linkText</a>";
                }
        ...
        $this->convertKey('status', 'html', function ($value, $key, $item) use ($setStatusUrl, $that) {
            ...
            return "<a class='ajax-get' href=\"{$url}\" >$text</a>";
        });
